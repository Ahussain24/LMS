...
class IssueBookView(MethodView):
    def get(self):
        ...
        if books:
            return render_template(
                "issue.html", books=books, year=datetime.now().year
            )
        flash("No books are currently available!")
        return render_template(
            "issue.html", year=datetime.now().year, books=Book.query.all()
    ...
    def post(self):
        book_id = int(request.form.get("book"))
        book_copy = Copy.query.filter_by(book=book_id, issued_by=None).first()

        print(type(book_copy))
        if book_copy:
            ...
            book_copy.issued_by = current_user.id
            book_copy.date_issued = datetime.now()
            book_copy.date_return = datetime.now() + timedelta(days=1)
