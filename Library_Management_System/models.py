...
class User(UserMixin, db.Model):
    ...
    email = db.Column(db.String(255), unique=True)
    password = db.Column(db.String(255))
    copies = db.relationship("Copy", backref="issue", lazy=True)
    admin = db.Column(db.Boolean, default=False)
...
class Book(db.Model):
    ...
    author = db.Column(db.String(255))
    description = db.Column(db.Text)
    copies = db.relationship("Copy", backref=db.backref("posts", lazy=True), cascade="all,delete")
    total_copy = db.Column(db.Integer)
    issued_copy = db.Column(db.Integer)
    present_copy = db.Column(db.Integer)
...
copies = db.relationship("Copy", backref=db.backref("book", lazy=True))
